...
final class QuizFlowTests: XCTestCase {
    ...
    private func insertQuestion(
        ...
        midCategory: MidCategory = .publicPension,
    ...
    func testCorrectIndexPointsToCorrectChoiceAfterShuffle() {
        for label in ChoiceLabel.allCases {
            insertQuestion("FPTEST_PENSION_000\(label.rawValue)", correct: label)
        }
    ...
    func testShuffleKeepsAllChoices() {
        let question = insertQuestion("FPTEST_PENSION_0001")
        viewModel.configure(context: context)
        viewModel.startNewQuiz()
    ...
    func testShuffleProducesVaryingOrders() {
        insertQuestion("FPTEST_PENSION_0001")
        viewModel.configure(context: context)
    ...
    func testAnsweringShowsExplanationWithoutAdvancing() {
        insertQuestion("FPTEST_PENSION_0001")
        insertQuestion("FPTEST_PENSION_0002")
        viewModel.configure(context: context)
        viewModel.startNewQuiz()
    ...
    func testSetIsCappedAtQuestionCount() {
        for i in 1...25 {
            insertQuestion(String(format: "FPTEST_PENSION_%04d", i))
        }
        viewModel.configure(context: context)
        viewModel.startNewQuiz()
    ...
    /// 科目別の内訳が正しく集計される（次の出題範囲を決める材料なので結果画面の主表示）
    func testFieldResultsAreAggregated() {
        insertQuestion("FPTEST_PENSION_0001", midCategory: .publicPension)
        insertQuestion("FPTEST_SOCINS_0001", midCategory: .socialInsurance)
        insertQuestion("FPTEST_EQUITY_0001", midCategory: .equities)
        viewModel.configure(context: context)
        viewModel.startNewQuiz()
        ...
        XCTAssertEqual(byField[.lifePlanning]?.totalCount, 2, "公的年金と社会保険はどちらもライフプランニングと資金計画")
        XCTAssertEqual(byField[.lifePlanning]?.correctCount, 2)
        XCTAssertEqual(byField[.financialAssets]?.totalCount, 1)
        XCTAssertNil(byField[.realEstate], "出題されなかった科目は結果に出さない")
    }
    ...
    func testRetryMissedIgnoresScopeAndCap() {
        insertQuestion("FPTEST_PENSION_0001", difficulty: .advanced)
        insertQuestion("FPTEST_SOCINS_0001", midCategory: .socialInsurance, difficulty: .advanced)
        viewModel.configure(context: context)

        // 出題範囲を「金融資産運用のみ」に絞る。解き直しはこれを無視する必要がある
        var scope = StudyScope.default
        scope.setField(.financialAssets)
        StudySettings.studyScope = scope

        viewModel.startNewQuiz(scope: .retryMissed(questionIds: ["FPTEST_PENSION_0001", "FPTEST_SOCINS_0001"]))

        XCTAssertEqual(viewModel.phase, .inProgress)
        XCTAssertEqual(
            viewModel.questions.map(\.id),
            ["FPTEST_PENSION_0001", "FPTEST_SOCINS_0001"],
    ...
    func testAnswersAreRecordedToProgress() {
        ...
        let progress = ProgressRepository(context: context).allProgress()["FPTEST_PENSION_0001"]
