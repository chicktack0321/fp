...
///
/// 課金状態では絞らない。出題対象から外れている標準・応用問題も閲覧・解説の読み直しはできる
...
struct QuestionListView: View {
    ...
    private var filterBar: some View {
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 8) {
                Menu {
                    Button("すべての科目") { viewModel.filter.setField(nil) }
                    ForEach(ExamField.allCases) { field in
                        Button(field.displayName) { viewModel.filter.setField(field) }
                    }
                } label: {
                    chip("科目", viewModel.filter.field?.shortName ?? "すべて")
                }

                Menu {
                    Button("すべての細目") { viewModel.filter.midCategory = nil }
                    ...
                    chip("細目", viewModel.filter.midCategory?.displayName ?? "すべて")
                }
