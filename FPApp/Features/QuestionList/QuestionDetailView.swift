...
struct QuestionDetailView: View {
    ...
    private var metadata: some View {
        VStack(alignment: .leading, spacing: 6) {
            HStack(spacing: 6) {
                tag(question.field.shortName, tint: .blue)
            ...
            // 税制・社会保険の数値は毎年変わる。どの時点の制度に基づく問題かを必ず出す
            Text("法令基準日 \(LawBasisDate.display(question.lawBasisDate)) / \(question.questionId)")
                .font(.caption2)
