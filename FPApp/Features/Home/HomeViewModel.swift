...
final class HomeViewModel {
    ...
    /// 科目別習熟度。絞り込みの影響を受けず常に全問で集計する（学習全体の地図として見る数字なので）
    ...
    /// いちばん遅れている科目。次に何をやるべきかの案内に使う。
    ///
    /// まだどの科目も習得済みが0のうちは返さない。全科目が同率0%のときに最小値を取ると、
    /// 並び順で決まった科目を「いちばん遅れている」と言い切ることになり、根拠のない案内になる。
    ...
    /// 進んでいる実感が得られないため、科目・細目・難易度で切って見られるようにする。
    ...
    /// 科目別の習熟度。
    ///
    /// こちらは `masteryScope` の絞り込みを**適用しない**。学習全体の地図として見る数字なので、
    /// 「公的年金だけに絞ったときのライフプランニング」のような部分集合を出すと意味が変わる。
