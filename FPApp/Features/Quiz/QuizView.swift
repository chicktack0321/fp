...
struct QuizView: View {
    ...
    private var startScreen: some View {
        ScrollView {
            VStack(spacing: 16) {
                VStack(spacing: 6) {
                    Text("\(AppFlavor.current.grade)演習")
                        .font(.title2).bold()
        ...
        VStack(alignment: .leading, spacing: 16) {
            ...
            VStack(spacing: 10) {
                ForEach(Array(question.choices.enumerated()), id: \.element.id) { index, choice in
                    ChoiceButton(
                        label: question.displayLabel(at: index),
                        text: choice.text,
...
private struct QuizProgressCard: View {
    ...
    let total: Int
    let fraction: Double
    /// いまどの科目を解いているかは、解答の手掛かりではなく現在地の把握に効く
    ...
    var body: some View {
        Button(action: action) {
            HStack(alignment: .firstTextBaseline, spacing: 10) {
                // ○×式では記号を出さない（`QuizQuestion.displayLabel` が空文字を返す）。
                // 空の Text でも幅18ptを確保してしまうため、要素ごと落とす
                if !label.isEmpty {
                    Text(label)
