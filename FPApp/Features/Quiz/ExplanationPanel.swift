...
struct ExplanationPanel: View {
    ...
    /// 「選んだ選択肢（A）」のように記号を添える。○×式では記号を出さないので、
    /// 括弧ごと落とす（`QuizQuestion.displayLabel` が空文字を返す）
    private func heading(_ base: String, at index: Int) -> String {
        let label = question.displayLabel(at: index)
        return label.isEmpty ? base : "\(base)（\(label)）"
    }

    /// 「正解: A. 〜」の先頭。記号が無いときは選択肢の文言だけにする
    private func correctPrefix() -> String {
        let label = question.displayLabel(at: question.correctIndex)
        ...
        VStack(alignment: .leading, spacing: 14) {
            ...
            if !isCorrect, let selected = selectedChoice {
                choiceExplanationBlock(
                    heading: heading("選んだ選択肢", at: selectedIndex),
        ...
        HStack(alignment: .top, spacing: 10) {
            ...
            VStack(alignment: .leading, spacing: 3) {
                ...
                if let correctChoice {
                    Text("正解: \(correctPrefix())\(correctChoice.text)")
                        .font(.subheadline)
    ...
    /// 全選択肢の解説。
    ///
    /// 誤答選択肢がなぜ誤りかを知ることが択一問題の学習価値の半分を占めるため必ず用意するが、
    /// 既定では畳んでおく。すべてを開いた状態にすると、いちばん重要な
                    ...
                    ForEach(Array(question.choices.enumerated()), id: \.element.id) { index, choice in
                        VStack(alignment: .leading, spacing: 3) {
                            HStack(alignment: .firstTextBaseline, spacing: 6) {
                                if !question.displayLabel(at: index).isEmpty {
                                    Text(question.displayLabel(at: index))
                                        .font(.caption).bold()
    ...
    private var metadata: some View {
        VStack(alignment: .leading, spacing: 6) {
            HStack(spacing: 6) {
                // 科目名は「ライフプランニングと資金計画」のように長い。
                // 細目と難易度と3つ並べる場所なので短縮名を使う
                tag(question.question.field.shortName, tint: .blue)
