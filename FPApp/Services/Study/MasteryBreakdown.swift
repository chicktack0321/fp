...
/// 過去に遡って再計算できない）。合計値だけを焼いていると後から科目や難易度で分けられないため、
/// 「細目×難易度」のセルごとに数えて保存する。
/// 単一軸の合計を持つ方式では「公的年金かつ応用」のような組み合わせを復元できない。
///
/// 科目ではなく細目をキーにしているのは、科目は細目から一意に決まるため。
/// 科目で焼くと細目での絞り込みが後から作れない。
