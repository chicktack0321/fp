...
final class Entitlements {
    ...
    private let logger = Logger(subsystem: AppFlavor.current.bundleIdentifier, category: "Entitlements")
    ...
    var availableDifficulties: Set<QuestionDifficulty> { rights.availableDifficulties }
    ...
    private func loadProduct() async {
        do {
            product = try await Product.products(for: [AppFlavor.current.unlockProductID]).first
        } catch {
    ...
    func refreshPurchaseState() async {
        ...
        for await result in Transaction.currentEntitlements {
            ...
            guard transaction.productID == AppFlavor.current.unlockProductID,
